...
\begin{proof}
    ...
    \begin{align*}
        ...
        C^{(2)}(x) &= 3n+2 = 3\left(\frac{x-1}{2}\right)+2
            = \frac{3x-3}{2}+2 = \frac{3}{2}x + \frac{1}{2}
    \end{align*}
    \[
        \implies C^{(2)}(x) < \frac{3}{2}x + 1.
    \]

    \textbf{Case 2: $x$ is even, $x = 2m$.}
    \begin{align*}
        C(x) &= m, \quad \text{($m$ is odd)} \\
        C^{(2)}(x) &= 3m+1 = 3\left(\frac{x}{2}\right)+1 = \frac{3}{2}x+1
    \end{align*}
    \[
        \implies C^{(2)}(x) = \frac{3}{2}x + 1.
    \]

    So, $C^{(2)}(x) \le \dfrac{3}{2}x + 1$.
\end{proof}
...
\begin{solution}[(b)]
    ...
    $u,v\in\emptyset$ with $u\ne v$. Hence the implication
    ``$u\ne v \implies (u,v)$ is edge'' holds vacuously for all
    $u,v\in\emptyset$. So $\emptyset$ is a clique, i.e.\
    $\emptyset\in\I$.

    \textbf{(I2).} Need to prove: if $X\subseteq Y$ and $Y$ is a
    clique, then $X$ is a clique.

    For all $u,v\in X$ with $u\ne v$: since $X\subseteq Y$, we have
    $u,v\in Y$ with $u\ne v$. Since $Y$ is a clique, $(u,v)$ is an
    edge. Hence $X$ is also a clique.

    \textbf{(I3) fails in general.} Consider the graph with vertices
    $\{a,b,c,d\}$ and a single edge $(b,c)$. Let $X=\{a\}$ and
    $Y=\{b,c\}$.

    $X$ and $Y$ are cliques. $Y\setminus X = \{b,c\}$:
    \begin{itemize}
        \item $X\cup\{b\} = \{a,b\}$ is not a clique, since $(a,b)$
            is not an edge.
        \item $X\cup\{c\} = \{a,c\}$ is not a clique, since $(a,c)$
            is not an edge.
    \end{itemize}
\end{proof}
...
\begin{solution}[(c)]
    ...
    $\nexists\, Y\in\I$ such that $X\subsetneq Y$.

    We need to prove: $\forall X,Y\in\I$, if $X$ maximal and $Y$
    maximal, then $|X|=|Y|$.

    Assume, for contradiction, there exist two maximal independent
    sets $X,Y\in\I$ with $|X|\ne|Y|$. WLOG, assume $|X|<|Y|$.

    Since $X,Y\in\I$ and $|X|<|Y|$, there exists $y\in Y\setminus X$
    such that $X\cup\{y\}\in\I$.

    Since $y\notin X$, we have $X\subsetneq X\cup\{y\}$, and
    $X\cup\{y\}\in\I$. This contradicts the assumption that $X$ is
    maximal.
...
\begin{solution}[(e)]
    ...
    $w(e_i) \ge w(e_{i+1})$ for all $1\le i<r$.

    Note that $e_i$ is the maximum-weight element among all
    $y\in E\setminus X_{i-1}$ satisfying $X_{i-1}\cup\{y\}\in\I$,
    i.e.\
    \[
        X_{i-1}\cup\{e_i\}\in\I, \qquad
        \forall y \text{ with } X_{i-1}\cup\{y\}\in\I,\ w(e_i)\ge w(y).
    \]

    Assume, for contradiction, that $w(e_i) < w(e_{i+1})$ for some
    $1\le i<r$.
    ...
    \[
        X_i \cup \{e_{i+1}\} \in \I.
    \]
    Since $X_{i-1}\subseteq X_i$, we have
    ...
    Therefore, the assumption is false, and $w(e_i)\ge w(e_{i+1})$
    for all $1\le i<r$.
\end{proof}
...
\begin{solution}[(f)]
    ...
    there is no $y\in E\setminus X$ such that $X\cup\{y\}\in\I$. So
    $X$ is a maximal independent set.

    By part (c), we know $|X|=|Y|=r$. Write $Y=\{f_1,\dots,f_r\}$
    with $w(f_i)\ge w(f_{i+1})$ for $1\le i\le r$.

    We need to prove that for each $i=1,\dots,r$,
    $w(e_i)\ge w(f_i)$.

    By contradiction (minimal counterexample): assume there exists a
    minimum $i$ such that $w(e_i) < w(f_i)$. By minimality of $i$, we
    have $w(e_k)\ge w(f_k)$ for all $k<i$.

    Let $X_{i-1}=\{e_1,\dots,e_{i-1}\}$ and $F_i=\{f_1,\dots,f_i\}$.
    ...
    $i$ exists, and $w(e_i)\ge w(f_i)$ for all $i=1,\dots,r$.
    ...
    $w(X)\ge w(Y)$ forces $w(X)=w(Y)$. Therefore $X$ is also a
