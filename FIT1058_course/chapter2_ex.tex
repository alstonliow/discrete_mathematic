...
\paragraph{Setup: grouping by length.} For each $n\ge0$, there are $2^n$
bitstrings of length $n$. We assign each bitstring a natural number
(``index'') as follows:

\begin{center}
\begin{tabular}{c c c c}
\hline
length & bitstring & number of bitstrings & index $\in \N$ \\
\hline
$0$ & $\varepsilon$ & $1$ & $0$ \\
$1$ & $0,1$ & $2=2^1$ & $1,2$ \\
$2$ & $00,01,10,11$ & $2^2$ & $3,4,5,6$ \\
...
\paragraph{Defining $f_n$.} For $\{0,1\}^n$, the value of a string is a
member of $\{0,1,\dots,2^n-1\}$. Define $f_n$ to be the value of
$\{0,1\}^n$:
\[
    f_n : \{0,1\}^n \longrightarrow \{0,1,\dots,2^n-1\}, \qquad
    f_n(s) = f_n(s_1 s_2 \cdots s_n) = \sum_{i=1}^n s_i\cdot 2^{n-i}.
\]

\begin{claim}
    For every $n$, $f_n$ is a bijection.
\end{claim}
\begin{proof}
    When $n=0$: $f_0:\{\varepsilon\}\to\{0\}$, $f_0(\varepsilon)=0$, is a
    bijection.

    Assume $f_{n-1}:\{0,1\}^{n-1}\to\{0,\dots,2^{n-1}-1\}$ is a bijection.

    For $f_n$,
    \[
        f_n(s_1\cdots s_n) = s_1\cdot 2^{n-1} + \sum_{i=2}^n s_i\cdot 2^{n-i}
        = s_1\cdot 2^{n-1} + f_{n-1}(s_2\cdots s_n).
    \]

    When $s_1=0$: $f_n(s_1\cdots s_n)\in\{0,\dots,2^{n-1}-1\}$.

    When $s_1=1$: $f_n(s_1\cdots s_n) \in \{2^{n-1},\dots,2^{n-1}-1+2^{n-1}\}
    = \{2^{n-1},\dots,2^n-1\}$.

    We know that $f_n(s_1\cdots s_n)\in\{0,\dots,2^n-1\}$.

    By induction, we know that $f_n$ is a bijection.
\end{proof}

\paragraph{Starting index of each length group.} Observe that, for
length $=n$, the starting index is equal to the number of bitstrings of
length $=0$ to length $=n-1$:
\[
    \text{starting index} = \sum_{k=0}^{n-1} |\{0,1\}^k| = \sum_{k=0}^{n-1} 2^k
    = \frac{2^n-1}{2-1} = 2^n-1.
\]

So, $f : \{0,1\}^* \to \N$ is equal to $(2^n-1) + $ (position of bitstring
in the group of the length):
\[
    f(s) = (2^n-1) + f_n(s), \qquad n = |s|.
\]
...
\end{claim}
\begin{proof}
    We know $f(s) \in \big[\,2^{|s|}-1,\ 2^{|s|+1}-2\,\big]$.

    \textbf{First, we prove $f$ is injective.}

    \emph{Case 1: $s\ne t$ and $|s|\ne|t|$.} Let $|s|<|t|$. Then
    \[
        f(s) \in \big[2^{|s|}-1,\ 2^{|s|+1}-2\big], \qquad
        f(t) \in \big[2^{|t|}-1,\ 2^{|t|+1}-2\big].
    \]
    Since $|s|<|t|$ are integers, $|s|+1\le|t|$, so
    \[
        2^{|s|+1}-2 \le 2^{|t|}-2 < 2^{|t|}-1.
    \]
    So $\big[2^{|s|}-1,\ 2^{|s|+1}-2\big] \cap \big[2^{|t|}-1,\ 2^{|t|+1}-2\big]
    = \varnothing$. Hence $f(t)\ne f(s)$.

    \emph{Case 2: $s\ne t$ and $|s|=|t|$.}
    \[
        f_{|s|}(s) \ne f_{|t|}(t) \implies f(s) \ne f(t).
    \]

    By Case 1 and 2, $f$ is injective.

    \textbf{Now we prove $f$ is surjective}, i.e.\ need to prove
    $\forall m\in\N,\ \exists\, s\in\{0,1\}^*,\ f(s)=m$.

    We know $f(s) \in \big[2^{|s|}-1,\ 2^{|s|+1}-2\big]$, so we need
    \[
        2^{|s|}-1 \le m \le 2^{|s|+1}-2
        \iff
        2^{|s|} \le m+1 \le 2^{|s|+1}-1
        \iff
        2^{|s|} \le m+1 < 2^{|s|+1}
        \iff
        |s| \le \log_2(m+1) < |s|+1.
    ...
    So let $n = \lfloor \log_2(m+1) \rfloor$. Then
    \[
        2^n \le m+1 < 2^{n+1}
        \implies
        0 \le m - 2^n + 1 < 2^n.
    ...
    So $m-2^n+1 \in [0,2^n) \subseteq [0,2^n-1]$, which is the domain of
    $f_n^{-1}$ (i.e., $m-2^n+1$ is in the range of $f_n$).

    $f_n$ is surjective, so $\exists\, s\in\{0,1\}^n$ such that
    $f_n(s) = m-2^n+1$. Then
    \[
        f(s) = (2^n-1) + f_n(s) = (2^n-1) + (m-2^n+1) = m.
    \]

    We proved that $\forall m\in\N$, $\exists\, s\in\{0,1\}^*$, $f(s)=m$.
    So, $f$ is surjective.
...
\end{solution}
\textbf{First Attempt (injective but \textbf{not} surjective --- kept for reference):}

\begin{solution}[(b), Attempt 1]

\paragraph{Setup.} Let $x=(x_1,\dots,x_k)\in\{0,1\}^{*+}$ be a tuple of
bitstrings, $x_i\in\{0,1\}^*$. Direct concatenation of the $x_i$ is not
injective: e.g., $(01,1)$ and $(0,11)$ both concatenate to $011$. So each
$x_i$ must be encoded in a self-delimiting way before concatenation.

\paragraph{Encoding a single bitstring.} For $x_i = t_1t_2\cdots t_n$,
define
\[
    \phi(x_i) = \underbrace{t_1t_1\,t_2t_2\,\cdots\,t_nt_n\,01}_{2n+2 \text{ characters}}.
\]
Each original bit is doubled (\texttt{0}$\to$\texttt{00}, \texttt{1}$\to$\texttt{11}),
and the string is terminated with \texttt{01}, a pair that never occurs
among the doubled bits.

\paragraph{Encoding the tuple.} Define
\[
    g : \{0,1\}^{*+} \to \{0,1\}^*, \qquad
    g(x_1,\dots,x_k) = \phi(x_1)\,\phi(x_2)\,\cdots\,\phi(x_k).
\]

\textbf{Pseudocode for $g$:}

\begin{algorithm}[H]
    \DontPrintSemicolon
    \KwIn{a tuple $x = (x_1,\dots,x_k) \in \{0,1\}^{*+}$}
    \KwOut{a bitstring $\mathrm{result} \in \{0,1\}^*$}
    $\mathrm{result} \gets \varepsilon$\;
    \ForEach{$x_i \in x$}{
        \ForEach{$t \in x_i$}{
            $\mathrm{result} \gets \mathrm{result} \cdot t\, t$\;
        }
        $\mathrm{result} \gets \mathrm{result} \cdot 01$\;
    }
    \Return $\mathrm{result}$\;
    \caption{Encoding $g(x)$}
\end{algorithm}

\textbf{Pseudocode for the decoder $g^{-1}$:}

\begin{algorithm}[H]
    \DontPrintSemicolon
    \KwIn{a bitstring $s \in \{0,1\}^*$}
    \KwOut{a tuple $\mathrm{tuple}$}
    $\mathrm{tuple} \gets ()$\;
    $p \gets 0$\;
    $n \gets |s|$\;
    \While{$p < n$}{
        $\mathrm{current} \gets \varepsilon$\;
        \While{$\mathrm{true}$}{
            $\mathrm{pair} \gets s[p]\,s[p+1]$\;
            $p \gets p + 2$\;
            \eIf{$\mathrm{pair} = 01$}{
                \textbf{break}\;
            }{
                $\mathrm{current} \gets \mathrm{current} \cdot \mathrm{pair}[0]$ \tcp*{$00\to 0,\ 11\to 1$}
            }
        }
        $\mathrm{tuple} \gets \mathrm{tuple} \cdot (\mathrm{current})$\;
    }
    \Return $\mathrm{tuple}$\;
    \caption{Decoding $g^{-1}(s)$}
\end{algorithm}

Note that $|g(x)| = 2\sum_{i=1}^k |x_i| + 2k$, which is always even.

\begin{claim}
    $g$ is injective.
\end{claim}
\begin{proof}
    We show $g^{-1}(g(x)) = x$ for all $x=(x_1,\dots,x_k)\in\{0,1\}^{*+}$,
    by induction on $k$.

    \textbf{Lemma.} After decoding $\phi(x_i)$, the position lands exactly
    at the start of the next block. This is because each pair $t_jt_j \in
    \{00,11\}$ is never equal to the terminator $01$, so decoding of a
    block cannot stop early; it stops exactly at the appended $01$.

    \textbf{Base case ($k=1$):} $g(x)=\phi(x_1)$, so by the Lemma,
    $g^{-1}(g(x)) = (x_1) = x$.

    \textbf{Inductive step:} Assume the statement holds for tuples of
    length $k-1$. For $x=(x_1,\dots,x_k)$,
    \[
        g(x) = \phi(x_1)\cdot\big[\phi(x_2)\cdots\phi(x_k)\big].
    \]
    By the Lemma, decoding recovers $x_1$ and leaves the position at the
    start of $\phi(x_2)\cdots\phi(x_k) = g(x_2,\dots,x_k)$. By the
    inductive hypothesis, decoding this remainder recovers
    $(x_2,\dots,x_k)$. Hence $g^{-1}(g(x)) = x$.

    By induction, $g^{-1}(g(x))=x$ for all $x\in\{0,1\}^{*+}$.

    Now suppose $x,y\in\{0,1\}^{*+}$, $x\ne y$, and $g(x)=g(y)$. Then
    \[
        x = g^{-1}(g(x)) = g^{-1}(g(y)) = y,
    \]
    a contradiction. So $g(x)\ne g(y)$, i.e., $g$ is injective.
\end{proof}

\begin{claim}
    $g$ is \textbf{not} surjective.
\end{claim}
\begin{proof}
    Since $\{0,1\}^{*+}$ requires nonempty tuples, the smallest possible
    tuple is $(x_1)$ with $x_1=\varepsilon$, giving
    \[
        \min_{x\in\{0,1\}^{*+}} |g(x)| = 2|x_1|+2 = 2(0+1) = 2.
    \]
    More importantly, $|g(x)| = 2\sum_i|x_i|+2k$ is \emph{always even}, for
    every $x\in\{0,1\}^{*+}$. So $g$ never outputs a string of odd length.

    For example, no $x$ satisfies $g(x)=0$ or $g(x)=1$ (both length $1$,
    odd). Hence there exist strings in $\{0,1\}^*$ not in the image of
    $g$, so $g$ is not surjective.
\end{proof}

\textbf{Conclusion.} Since $g$ is injective but not surjective, $g$ is
\textbf{not} a bijection. This encoding scheme (self-delimiting via bit
doubling) fundamentally cannot reach odd-length strings, no matter how the
terminator is chosen, since every encoded block contributes an even
number of characters. A different construction is needed for a genuine
bijection $\{0,1\}^{*+}\to\{0,1\}^*$.
